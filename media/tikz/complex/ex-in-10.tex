...
\begin{tikzpicture}
    ...
    \begin{scope}
        ...
        \draw[color=gray!30, fill=gray!30] (-0.5*\ymin-0.75,\ymin) -- (-0.5*\ymax-0.75,\ymax) -- (\xmin,\ymax) -- (\xmin,\ymin) -- cycle;;
    \end{scope}
    ...
    % y-0.5 = -2(x+1) -> y = -2x -1.5
    \draw[ultra thick, dashed] (-0.5*\ymin-0.75,\ymin) -- (-0.5*\ymax-0.75,\ymax);

    \filldraw[blue] ( 0,1) circle (2pt) node[above right] {$i$};
