...
%   }
    ...
    % Parabola
    \draw[thick, black, rotate=\alpha] plot[parametric, smooth, domain=-5:5] ({\xc+\x}, {\yc+0.25*\x*\x});
    % Focus
    \filldraw[blue, rotate=\alpha] (0,1) circle (2pt) node[below left] {$F$};
    % Directrix
    \draw[red, rotate=\alpha] (-10,-1) -- (10,-1);
    \filldraw[red, rotate=\alpha] (5,-1) circle (0) node[below] {$d$};
    % Semi-latus rectum
    \draw[ultra thin, orange, rotate=\alpha] ( 0, 1) -- ( 2, 1) node[midway, above, rotate=\alpha] {$ed = d$};
    \draw[ultra thin, orange, rotate=\alpha] ( 2, 1) -- ( 2,-1) node[midway, right] {$d$};
